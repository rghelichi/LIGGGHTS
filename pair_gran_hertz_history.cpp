...
void PairGranHertzHistory::compute_force_eval(int eflag, int vflag,int addflag)
{
  ...
  */
  ...
  myfile.open("fcb.dat");//,std::ios::app);
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    //double iii=tag[i];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      j &= NEIGHMASK;
      int jjj=tag[j]-1;  
      //if(jjj>inum) std::cerr<<"TAG in J="<<tag[j]<<std::endl;
	...
	if (fluid_bonds[i].count(j) != 0) {	
		cbflag=true;
		vol_bond_ij0 = fluid_bonds[i][j].first;	      
		...
		if(deltan>0)
			...
			{fcb=-fcbConst;}
		vol_bond_ij=vol_bond_ij0*(1- exp(-c0*pow(thickeff,3)/vol_bond_ij0/viscb*time_bond_ij));
		if(thick_flag[iii][jjj]==1){
			thickness[iii]=thicki-vol_bond_ij*radj/(radj+radi)/4/PI/pow(radi,2);
			thickness[jjj]=thickj-vol_bond_ij*radi/(radj+radi)/4/PI/pow(radj,2);
			thickj=thickness[jjj];
			thicki=thickness[iii];
      ...
      else
      {
		...
		//myfile<<fcb<<","<<deltan<<",";	
			...
			// relative translational velocity
				...
				fluid_bonds[i][j] = std::make_pair(6*thickeff/reff*reff3*4*PI/3, timestep);//update->ntimestep * update->dt);
			}	
