...
static std::vector<double> thickness;
static std::vector<std::map<int ,std::pair<double, double> > >fluid_bonds;

/* ---------------------------------------------------------------------- */

PairGranHertzHistory::PairGranHertzHistory(LAMMPS *lmp) : PairGran(lmp)
{
...
PairGranHertzHistory::~PairGranHertzHistory()
{
    memory->destroy(Yeff);
    memory->destroy(Geff);
...
void PairGranHertzHistory::history_args(char** args)
{
...
inline void PairGranHertzHistory::addCohesionForce(int &ip, int &jp,double &r, double &Fn_coh) 
{
...
void PairGranHertzHistory::compute_force(int eflag, int vflag,int addflag)
{
...
void PairGranHertzHistory::compute_force_eval(int eflag, int vflag,int addflag)
{
  ...
  double radi,radj,radsum,rsq,r,rinv,rsqinv,reff,thicki,thickj;
  ...
  double volcb;
  double thetacb=40.0*PI/180;
  double scb;
  double viscb=1;
  double gammacb=10;
  double volfluid;
  double time_bond_ij;
  double volume_percent=0.05;
  double c0=1000;
  double vol_bond_ij;
  ...
  double inum_last=thickness.size();
  //if (update->ntimestep == update->firststep or inum_last != inum) {
  if (inum_last != inum) {	
  ...
  //std::ofstream myfile;
  //if(update->ntimestep == update->firststep) if (std::ifstream("fcb.dat")) remove("fcb.dat");
   int to_file_flag =0;
  std::ofstream thick_res;
  std::ofstream bridge_res;
  std::ofstream volCheck;
  volCheck.open("volcheck.dat",std::ios::app);
  
  if(update->ntimestep % 5000==0){
	to_file_flag =1;
	std::string thick="Bridge/Thick";
	std::string timestr=std::to_string(update->ntimestep);
	thick.append(timestr);
	thick.append(".dat");
	
	std::string bridge="Bridge/Bridge";
	bridge.append(timestr);
	bridge.append(".dat");
  
	
	thick_res.open(thick);//,std::ios::app);
  
	
	bridge_res.open(bridge);
  }
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    //double iii=tag[i];
    //if(i>inum) std::cerr<<"ERRORRR IN I="<<i<<std::endl;
    int iii=tag[i]-1;  
    thicki=thickness[iii];    	
    xtmp = x[i][0];
    ytmp = x[i][1];
    ztmp = x[i][2];
    if(to_file_flag)  thick_res<<iii<<","<<xtmp<<","<<ztmp<<","<<thicki<<std::endl;	  
    radi = radius[i];
    touch = firsttouch[i];
    ...
    //if(jnum==0) myfile<<0.0<<","<<0.0<<",";
    //std::cerr<<"jnum="<<jnum<<std::endl;	   
    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      j &= NEIGHMASK;
      int jjj=tag[j]-1;  
      //std::cerr<<"TAG in J="<<jjj<<std::endl;
      thickj=thickness[jjj]; 
      //std::cerr<<"Thickness="<<thickeff<<std::endl;	   
      delx = xtmp - x[j][0];
      dely = ytmp - x[j][1];
      delz = ztmp - x[j][2];
      ...
      double reffcb=2*reff;
      double reff3=pow(reffcb,3.0);	 
       scb=0;
       bool cbflag=false;
       double fcbConst=0;
       double Hb_D;
	time_bond_ij=0;
	double thickeff=(thicki+thickj)/2;
	//std::cout<<thickeff<<std::endl;
	if (fluid_bonds[i].count(j) != 0) {	
		...
		//	if (thicki<0) std::cerr<<iii<<"   "<<thickness[iii]<<"   "<<thicki<<"   "<<vol_bond_ij*radj/(12*(radj+radi)*4*PI*pow(reff,2))<<std::endl;
			...
			if(to_file_flag)  bridge_res<<thicki <<","<<thickj<<std::endl;
		}
		//std::cerr<<"V0="<<vol_bond_ij0<<",Time="<<timestep<<",j="<<jjj<<",Thickness="<<thickness[jjj]<<std::endl;
		double del_star=0.01;
		deriveContactModelParams(i,j,meff, del_star,kn,kt,gamman,gammat,xmu,rmu,vnnr); 	 
		double s_star=pow(sqrt(del_star)*fcbConst/kn,2.0/3.0);
	        if(deltan>=-s_star)	
			{Hb_D=1.0/sqrt(vol_bond_ij/s_star/s_star/PI/reff+1);
			 fvis= -6.0*PI*viscb*reff*reff*(1-Hb_D)*(1-Hb_D)*vnnr/s_star;
			}	
		else {Hb_D=1.0/sqrt(vol_bond_ij/deltan/deltan/PI/reff+1);
			 fvis=-6.0*PI*viscb*reff*reff*(1-Hb_D)*(1-Hb_D)*vnnr/(-deltan);
			}		
		scb=4*reff*(1+thetacb/2) *(pow(vol_bond_ij/reff3,1.0/3.0)+0.1*pow(vol_bond_ij/reff3,2.0/3.0)); 
		//myfile<<iii<<","<<jjj<<","<<fvis<<","<<fcb<<std::endl;	      
	...
	if (cbflag) 
		{
		thickness[iii]=thicki+vol_bond_ij/2;
		thickness[jjj]=thickj+vol_bond_ij/2;	
		fluid_bonds[i].erase(j);
		}
      ...
      else
      {
		...
		//fvis=0;
	      ...
	      if(rsq>radsum*radsum){
			...
			//std::cout<<"cbflag="<<cbflag<< "\n";
			if(cbflag) 
				{				
				//sHat=deltan*sqrt(reffcb/vol_bond_ij);   
				//fcb= -fcbConst/(1-1.05*sHat+2.5*sHat*sHat); WILLET
			
			fx = delx*fcb *rinv+fvis*rsqinv*delx;
			fy = dely*fcb *rinv+fvis*rsqinv*dely;
			fz = delz*fcb *rinv+fvis*rsqinv*delz;
			//std::cout<<fx;      
			}
		//myfile<<fcb<<","<<deltan<<",";	
	       }
		...
		//		std::cerr<<thicki<<"  "<<thickj<<"  "<<thickeff<<"   "<<thickeff*pow(reff,2)*4*PI/12<<std::endl;
			...
			ccel = kn*(radsum-r)*rinv - damp+fcb*rinv+fvis*rsqinv;
			//myfile<<fvis<<","<<deltan<<",";	
			//myfile<<fcb<<","<<deltan<<",";
			if (cohesionflag) { 
			...
			if(computeflag)
			{
			    //int iii=tag[i]-1;
			    //int jjj=tag[j]-1;		
			   // myfile<<iii<<","<<jjj<<","<<f[i][0]<<","<<f[i][1]<<","<<f[i][1]<<std::endl;	  
			...
			if (j < nlocal && computeflag) {
			  ...
			  //myfile<<iii<<","<<jjj<<","<<f[j][0]<<","<<f[j][1]<<","<<f[j][1]<<std::endl;	      
			  f[j][0] -= fx;
			  f[j][1] -= fy;
			  f[j][2] -= fz;
...
void PairGranHertzHistory::settings(int narg, char **arg) 
{
    ...
    while(iarg_ < narg && hasargs)
    {
        ...
        if (strcmp(arg[iarg_],"cohesion") == 0) {
            ...
            iarg_++;
            hasargs = true;
        } else if (force->pair_match("gran/Hertz/history",1) || force->pair_match("gran/hertz/history",1))
...
void PairGranHertzHistory::init_granular()
{
...
void PairGranHertzHistory::allocate_properties(int size)
{
    memory->destroy(Yeff);
    memory->destroy(Geff);
    memory->destroy(betaeff);
...
void PairGranHertzHistory::write_restart_settings(FILE *fp) 
{
...
void PairGranHertzHistory::read_restart_settings(FILE *fp) 
{
  if (comm->me == 0) {
    int readflag;
